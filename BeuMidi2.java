...
public class BeuMidi2 {

	static BeuMidi2 instance;
	...
	public BeuMidi2() throws Exception {
...
Util.debugDocument(document);
	...
	void removeTransposeElements(Document document) throws Exception {
		...
		for (int i = 0;  i < elementList.size();  ++i) {
			...
			for (Node child = transposeElement.getFirstChild();  child != null;  child = child.getNextSibling()) {
				transposeElement.getParentNode().insertBefore(child.cloneNode(true), transposeElement);
			}
			transposeElement.getParentNode().removeChild(transposeElement);
		}
	}
